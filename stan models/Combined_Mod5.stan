...
real bTemp;
real bTherm;
real<lower=00> sigma_MR; // error for MR model
...
resid_RSA[i] = RSA[i] - mu_RSA[i];
mu_MR[i] = aMR + bResid * resid_RSA[i] + bMassMR * MMR[i] + bTherm * ThermoStrat[i] + bIntMassTS * MMR[i] * ThermoStrat[i] + bTemp * Temp[i];
resid_MR[i] = MR[i] - mu_MR[i];
}}
